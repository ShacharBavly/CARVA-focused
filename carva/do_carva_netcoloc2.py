...
if __name__=='__main__':
    ...
    t.start('Load seeds')
    # Load the seed genes
    #common_seeds = Seeds(inputdata = os.path.join(args.indir, "lupus_cv2.txt"))
    common_seeds = Seeds(inputdata = os.path.join(args.indir, "ovary_cv2.txt"))
    rare_seeds = Seeds(inputdata = os.path.join(args.indir, "lupus_rv2.txt"))
    suffix = create_file_suffix(args.quant, args.transform, args.normalization, args.suffix)

    t.end('Load seeds')

    #shachar add: make it so non existing dirs will be created
    ...
    if (len(common_seeds.genes) < args.min_genes) or (len(rare_seeds.genes) < args.min_genes):
        ...
        with open(os.path.join(args.outdir, 'not_enough_seeds.txt'), 'a') as f:
            f.write(f"lupus_rare\t{n_rare}\tlupus_common\t{n_common}\tinput\n")
        ...
        if (len(common_seeds.genes) < args.min_genes) or (len(rare_seeds.genes) < args.min_genes):
            ...
            n_common = len(common_seeds.genes)    
            with open(os.path.join(args.outdir, 'not_enough_seeds.txt'), 'a') as f:
                f.write(f"lupus_rare\t{n_rare}\tlupus_common\t{n_common}\tnetwork\n")
        else:
            ...
            stats = {"trait_rare": "lupus_rare", "trait_common": "lupus_common", 'network':args.net_name, 'transform':args.transform, 'normalization':args.normalization}
            t.start('Common variants')

            ## Analyze the Common Genes using Q-NetColoc or B-NetColoc
            if args.quant:
                if exists(os.path.join(args.outdir, "lupus_common" + f'_zrv{suffix}.tsv')): # check if already calculated.
                    z_common=pd.read_csv(os.path.join(args.outdir, "lupus_common" + f'_zrv{suffix}.tsv'), sep="\t", index_col=0, header=None).squeeze('columns')
                ...
                #perform transformation and normalization of scores
                    ...
                    z_common.to_csv(os.path.join(args.outdir, "lupus_common" + f'_zrv{suffix}.tsv'), sep="\t", header=False)
                    z_common=pd.read_csv(os.path.join(args.outdir, "lupus_common" + f'_zrv{suffix}.tsv'), sep="\t", index_col=0, header=None).squeeze('columns')
                    z_common.index.name=None
            ...
            if args.quant:
                if exists(os.path.join(args.outdir, "lupus_rare" + f'_zrv{suffix}.tsv')):
                    z_rare = pd.read_csv(os.path.join(args.outdir, "lupus_rare" + f'_zrv{suffix}.tsv'), sep="\t", index_col=0, header=None).squeeze('columns')
                    ...
                    z_rare.to_csv(os.path.join(args.outdir, "lupus_rare" + f'_zrv{suffix}.tsv'), sep="\t", header=False)
                    z_rare = pd.read_csv(os.path.join(args.outdir, "lupus_rare" + f'_zrv{suffix}.tsv'), sep="\t", index_col=0, header=None).squeeze('columns')
                    z_rare.index.name=None
            ...
            with open(os.path.join(args.outdir, f'{stats_pref}_lupus_rare_lupus_common_{suffix}{stat_suffix}.txt'), 'w') as f:
